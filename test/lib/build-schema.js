...
// Real Supabase always ships a `storage` schema (storage.buckets,
// storage.objects, plus the storage.foldername()/storage.filename() helpers
// every Storage RLS policy in sql/ uses) — a bare cluster has none of it.
// Minimal stand-in, just enough to let a migration's `insert into
// storage.buckets` and its storage.objects RLS policies apply for real here,
